...
void RayTracing::CreateRaytracingRootSignatures()
{
	...
	// Create a global root signature shared across all raytracing shaders
	{
		...
		HRESULT hr = DXRDevice->CreateRootSignature(1, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(GlobalRaytracingRootSig.GetAddressOf()));
	}
}
...
void RayTracing::CreateRaytracingPipelineState(std::wstring raytracingShaderLibraryFile)
{
	...
	//REMOVED - Local root signature
	//REMOVED - Association of local root sig to shader
...
MeshRayTracingData RayTracing::CreateBottomLevelAccelerationStructureForMesh(Mesh* mesh)
{
	...
	geometryDesc.Triangles.VertexBuffer.StrideInBytes = mesh->GetVertexView().StrideInBytes;
	...
	geometryDesc.Triangles.IndexFormat = mesh->GetIndexView().Format;
...
void RayTracing::CreateTopLevelAccelerationStructureForScene(std::vector<std::shared_ptr<Entity>> scene)
{
	...
	{
		std::shared_ptr<Entity> current = scene[i];

		// Grab the entity's transform and transpose to column major
		DirectX::XMFLOAT4X4 transform = current->GetTransform().GetWorldMatrix();
		...
		raytracingInstanceDescs.push_back(instanceDesc);
	}
	...
	const size_t SceneSize = sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * raytracingInstanceDescs.size();
	...
	if (SceneSize > tlasInstanceDataSizeInBytes[frameIndex])
	{
		// Reset and save the new size
		TLASInstanceDescBuffer[frameIndex].Reset();
		tlasInstanceDataSizeInBytes[frameIndex] = SceneSize;
	...
	HRESULT hr = TLASInstanceDescBuffer[frameIndex]->Map(0, 0, (void**)&mapped);
	memcpy(mapped, raytracingInstanceDescs.data(), SceneSize);
	...
	unsigned int a_uCubemapIndex)
{
	...
	// Final copy
	{
		...
		for (int i = 0; i < TemporalFilterationFrameCount; i++)
		{
			DXRCommandList->CopyResource(tempResource.Get(), RaytracingOutput.Get());
		}
...
void RayTracing::CreateEntityDataBuffer(std::vector<std::shared_ptr<Entity>> scene)
{
	...
	for (int i = 0; i < scene.size(); i++)
	{
		...
		data.Color = DirectX::XMFLOAT4(c.x, c.y, c.z, 1.0f);
		...
		TextureSet textures = mat->GetTextureSet();
		data.AlbedoIndex = textures.AlbedoIndex;
